...
app.get('/admin', async (req, res) => {
  ...
  res.send(`
    ...
    <style>
      ...
      table { width: 100%; border-collapse: collapse; background: white; border-radius: 10px; box-shadow: 0 5px 20px rgba(0, 0, 0, 0.1); }
    ...
    <body>
      ...
      <table>
        <thead>
          <tr>
            <th>الاسم</th><th>الجوال</th><th>الجهاز</th><th>السعر كاش</th><th>السعر تقسيط</th><th>القسط</th><th>الكود</th><th>الوقت</th><th>الحالة</th><th>حذف</th>
          </tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>
      <script>
        function deleteOrder(id) {
          if (confirm("هل أنت متأكد أنك تريد حذف الطلب؟")) {
            fetch('/api/delete/' + id, { method: 'DELETE' })
              .then(res => {
                if (!res.ok) {
                  res.json().then(data => alert(data.error || "حدث خطأ"));
                } else {
                  location.reload();
                }
              });
